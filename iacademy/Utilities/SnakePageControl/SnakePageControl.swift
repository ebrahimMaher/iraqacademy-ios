...
@IBDesignable open class SnakePageControl: UIView {
    ...
    @IBInspectable open var currentPage: CGFloat = 0 {
        didSet {
            layoutActivePageIndicator(currentPage)
        }
    }
    ...
    @IBInspectable open var activeTint: UIColor = .Blue_Brand_500 {
        didSet {
            activeLayer.backgroundColor = activeTint.cgColor
        }
    }
    @IBInspectable open var inactiveTint: UIColor = .systemGray4 {
        didSet {
