...
class DRMPlayerViewController: UIViewController {
    ...
    init(videoURL: String, licenseURL: String? = nil, certificateURL: String? = nil) {
    ...
    private func fetchVariants() {
        ...
        URLSession.shared.dataTask(with: url) { data, _, error in
            guard let data = data, let playlist = String(data: data, encoding: .utf8) else { return }
            self.variants = HLSVariant.parseVariants(from: playlist, baseURL: url.deletingLastPathComponent())
        }.resume()
    }
    ...
    private func setupPlayer() {
        ...
        if let license = licenseURL, let certificate = certificateURL,
           let licenseURL = URL(string: license),
           let certificateURL = URL(string: certificate) {
            
            drmManager = FairPlayDRMManager(licenseURL: licenseURL, certificateURL: certificateURL)
    ...
    @objc private func didTapQuality() {
        ...
        alert.addAction(UIAlertAction(title: "Auto", style: .default, handler: { _ in
            self.switchQuality(to: self.videoURL)
            self.qualityButton.setTitle("Auto", for: .normal)
        }))
        for variant in variants {
            let rawResolution = variant.resolution?.trimmingCharacters(in: .whitespacesAndNewlines)
            var resolutionLabel = "غير معروف"
            if let res = rawResolution, let height = res.split(separator: "x").last {
                resolutionLabel = "\(height)p"
            }
            alert.addAction(.init(title: resolutionLabel, style: .default, handler: { _ in
                self.switchQuality(to: variant.url.absoluteString)
                self.qualityButton.setTitle(resolutionLabel, for: .normal)
            }))
        }
...
extension DRMPlayerViewController: PlayerViewController {
...
extension DRMPlayerViewController: QualitySwitchableController {
